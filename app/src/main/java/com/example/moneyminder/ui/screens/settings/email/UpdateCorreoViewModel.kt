...
@HiltViewModel
class UpdateCorreoViewModel @Inject constructor(
    ...
    fun updateUsuario(correo: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val usuarioActualizado = usuario?.copy(correo = correo)
            val response = updateUsuarioUsecases.updateUsuario(usuario!!.id, usuarioActualizado)
            if (response.isSuccessful) {
